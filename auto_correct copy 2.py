...
def team_available_for_away(team, the_date, rows, matches):
  def nbr_possible_home_slots(row):
    count = 0
    for the_date in _get_all_dates(rows):
...
def find_all_possible_dates(match, rows, matches, grounds):
...
def build_fixtures(rows, matches, grounds):
  ...
  while True:
    for match in matches:
      if match["Date"] != "":
        continue
      possible_dates, ground = find_all_possible_dates(match, rows, matches, grounds)
      #  if possible_dates are zero then go back previous stage and run with another date
      assert len(possible_dates) != 0
      match["possible_dates"] = possible_dates
      match["Ground"] = ground
    
    match, best_date = choose_match_with_lower_possibility(matches)

    # let's assign this match
    match["Date"] = best_date
    # random.choice(match["possible_dates"])
...
build_fixtures(rows, matches, grounds)
